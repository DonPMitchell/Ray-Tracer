...
//
//  Compact a sorted array by removing redundant (== to another) elements.  Return
//  size of compacted array.  Uses ML_IsEqual()
//
template <class T>
int
ML_Unique(T rgtItem[], int nItems)
{
    int iFirst, iNew;

    for (iNew = iFirst = 0; ++iFirst < nItems; iNew = iFirst) {
        if (ML_IsEqual(rgtItem[iFirst], rgtItem[iNew])) {
            while (++iFirst < nItems) {
                if (!ML_IsEqual(rgtItem[iNew], rgtItem[iFirst]))
                    rgtItem[++iNew] = rgtItem[iFirst];
            }
            return ++iNew;
        }
    }
