...
struct DisplayRGB {
    ...
    float       Luminance() const { return 0.212649390101432f * red
                                         + 0.715169131755828f * grn
                                         + 0.0721815153956413f * blu; }
    float       Norm() const { return (red + grn + blu)/3.0f; }
...
extern const DisplayRGB    ML_Cyan;
extern const DisplayRGB    ML_Yellow;
extern const DisplayRGB    ML_Magenta;
extern const DisplayRGB    ML_Orange;
...
struct Image {
    ...
    int     NewImage(const Image &im) { return NewImage(im.m_nWidth, im.m_nHeight, im.m_nChannels,
    ...
    DisplayRGB  SplatRGB(const DisplayRGB &rgb, int iCol, int jRow);
    float       Sample(double xSample, double ySample, double fScale = 1.0, int kChan = 0) const;
    void        SampleGradientDSinc(float rgf[2], double xSample, double ySample, int kChan = 0) const;
    ...
    DisplayRGB  SplatRGB(const DisplayRGB &rgb, double xSample, double ySample, Image *pimWeight = 0, double fScale = 1.0);
    int         Image::Normalize(Image *pimWeight);
    double      SampleLerp(double x, double y, int k = 0) const;
    DisplayRGB  SampleLerpRGB(double x, double y) const;
    void        SampleGradient(float rgf[2], double xSample, double ySample, int kChan = 0) const
                    { rgf[0] = float(SampleLerp(xSample+0.5, ySample, kChan) - SampleLerp(xSample-0.5, ySample, kChan));
                      rgf[1] = float(SampleLerp(xSample, ySample+0.5, kChan) - SampleLerp(xSample, ySample-0.5, kChan)); }
    DisplayRGB  FastSplatRGB(const DisplayRGB &rgb, double x, double y, Image *pimWeight = 0);
    double      FastSplat(double f, double x, double y, Image *pimWeight = 0, int k = 0);     // SplatLerp
    double      GaussianSplat(double f, double x, double y, Image *pimWeight = 0, double fSigma = 1.0);
...
inline DisplayRGB
Image::SplatRGB(const DisplayRGB &rgb, int iCol, int jRow)
{
    int i, j;
