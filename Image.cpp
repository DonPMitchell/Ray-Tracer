...
//
//  Gradient filter and sampling
//
static float
Derivative4Filter(float x)
{
    ...
    x1 = F_PI*x;
    if (x < 0.001 && x > -0.001) {
        fDSinc = F_PI * x1*(-2.0f/6.0f + x1*x1*(4.0f/120.0f));
    } else {
        fDSinc = (cosf(x1) - sinf(x1)/x1)/x;
    }
    return fDSinc * fKaiser * float(gs_fI0Alpha); // * 
}

void
Image::SampleGradientDSinc(float rgfGrad[2], double xSample, double ySample, int kChan) const
{
    double x, y, xLow, xHigh, yLow, yHigh, fSumWeight, f, fDX, fDY;
    int i, j, iLow, jLow;
    float rgx[10], rgy[10], rgdx[10], rgdy[10];

    fSumWeight = fDX = fDY = 0.0;
    xLow  = xSample - KAISER4_HALFWIDTH;
    xHigh = xSample + KAISER4_HALFWIDTH;
    iLow = LOWEST_INDEX(xLow);
    yLow  = ySample - KAISER4_HALFWIDTH;
    yHigh = ySample + KAISER4_HALFWIDTH;
    jLow = LOWEST_INDEX(yLow);
    //
    //  Separable filter, store the y-dimension factors
    //
    for (j = jLow, y = INDEX_TO_SAMPLE(jLow); y <= yHigh; j++, y += 1.0) {
        rgy[j - jLow]  = Kaiser4Filter(float(y - ySample));
        rgdy[j - jLow] = Derivative4Filter(float(y - ySample));
    }
    for (i = iLow, x = INDEX_TO_SAMPLE(iLow); x <= xHigh; i++, x += 1.0) {
        rgx[i - iLow]  = Kaiser4Filter(float(x - xSample));
        rgdx[i - iLow] = Derivative4Filter(float(x - xSample));
    }
    for (i = iLow, x = INDEX_TO_SAMPLE(iLow); x <= xHigh; i++, x += 1.0) {
        for (j = jLow, y = INDEX_TO_SAMPLE(jLow); y <= yHigh; j++, y += 1.0) {
            fSumWeight += rgy[j - jLow] * rgx[i - iLow];
            f = Get(i, j, kChan);
            fDX += f * rgy[j - jLow] * rgdx[i - iLow];
            fDY += f * rgdy[j - jLow] * rgx[i - iLow];
        }
    }
    rgfGrad[0] = -float(fDX/fSumWeight);
    rgfGrad[1] = -float(fDY/fSumWeight);
}
...
const DisplayRGB    ML_Cyan(0.0, 1.0, 1.0);
const DisplayRGB    ML_Yellow(1.0, 1.0, 0.0);
const DisplayRGB    ML_Magenta(1.0, 0.0, 1.0);
const DisplayRGB    ML_Orange(1.0, 0.5, 0.0);
...
DisplayRGB
Image::SplatRGB(const DisplayRGB &rgb, double xSample, double ySample, Image *pimWeight, double fScale)
{
    double x, y, xLow, xHigh, yLow, yHigh, fWeightX, fWeight, fRecipScale, fHalfWidth;
    int i, j, iLow, jLow;
    float rgf[512];
...
double
Image::SampleLerp(double x, double y, int k) const
{
...
DisplayRGB
Image::SampleLerpRGB(double x, double y) const
{
    unsigned i1, i2, j1, j2;
    double dx, dy;
    DisplayRGB a, b, c, d;
...
DisplayRGB
Image::FastSplatRGB(const DisplayRGB &rgb, double x, double y, Image *pimWeight)
{
    int i1, i2, j1, j2;
    double dx, dy;
    ...
    j2 = j1 + 1;
    dy = y - floor(y);
    SplatRGB(rgb*(1.0-dx)*(1.0-dy), i1, j1);
    SplatRGB(rgb*(dx)*(1.0-dy),     i2, j1);
    SplatRGB(rgb*(1.0-dx)*(dy),     i1, j2);
    SplatRGB(rgb*(dx)*(dy),         i2, j2);
    if (pimWeight) {
        pimWeight->Splat((1.0-dx)*(1.0-dy), i1, j1);
        pimWeight->Splat((dx)*(1.0-dy),     i2, j1);
        pimWeight->Splat((1.0-dx)*(dy),     i1, j2);
        pimWeight->Splat((dx)*(dy),         i2, j2);
    }
    return rgb;
}

double
Image::FastSplat(double f, double x, double y, Image *pimWeight, int k)
{
    int i1, i2, j1, j2;
    double dx, dy;
    ...
    j2 = j1 + 1;
    dy = y - floor(y);
    Splat(f*(1.0-dx)*(1.0-dy), i1, j1, k);
    Splat(f*(dx)*(1.0-dy),     i2, j1, k);
    Splat(f*(1.0-dx)*(dy),     i1, j2, k);
    Splat(f*(dx)*(dy),         i2, j2, k);
    if (pimWeight) {
        pimWeight->Splat((1.0-dx)*(1.0-dy), i1, j1);
        pimWeight->Splat((dx)*(1.0-dy),     i2, j1);
        pimWeight->Splat((1.0-dx)*(dy),     i1, j2);
...
Gaussian(double x, double fSigma)
{
    return exp(-x*x/(2.0*fSigma*fSigma))/(ROOT2PI*fSigma);
}

double
Image::GaussianSplat(double f, double x, double y, Image *pimWeight,  double fSigma)
{
    int i, j, iWrap, i1, i2, j1, j2, N;
    double dx, dy, g1, g2, dr, dx2, dy2;

    // N = m_nWidth;
    N = 1;
    if (fSigma < 0.5/double(N))
        fSigma = 0.5/double(N);
    i1 = int(floor(double(N)*(x - DEV*fSigma)));
    i2 = int( ceil(double(N)*(x + DEV*fSigma)));
    j1 = int(floor(double(N)*(y - DEV*fSigma)));
    j2 = int( ceil(double(N)*(y + DEV*fSigma)));
    dr = DEV*DEV*fSigma*fSigma;
    for (i = i1; i <= i2; i++) {
        iWrap = i;
        if (iWrap < 0)
            iWrap += N;
        if (iWrap >= N)
            iWrap -= N;
        dx = (double(iWrap) + 0.5)/double(N);
        dx2 = (dx - x)*(dx - x);
        g1 = Gaussian(x - dx, fSigma);
        for (j = j1; j <= j2; j++) {
            dy = (double(j) + 0.5)/double(N);
            dy2 = (dy - y)*(dy - y);
            if (dx2 + dy2 > dr)
                continue;
            g2 = Gaussian(y - dy, fSigma);
            Splat(f*g1*g2, i, j);
            if (pimWeight)
                pimWeight->Splat(g1*g2, i, j);
        }
    }
    return f;
}
